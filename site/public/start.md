...
Any coding agent, with the generic skills CLI:

```
npx skills add matthewp/tideway
```

Claude Code, with the plugin marketplace:
...
Once installed, the `style` skill loads on its own whenever you write or edit CSS. Its full rules and the complete token tables live in the skill. Read them before writing styles.
